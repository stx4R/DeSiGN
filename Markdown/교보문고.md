...
> **대조 결과(2026-08-02).** KDS Color 페이지가 **텍스트로 발행하는 값은 4개뿐**이다 — 렌더해 확인한 표에 담긴 건 `blue 700 = #5055B1` ≈ `oklch(0.494 0.144 277)`(Informative) · `green 700 = #4DAC27` ≈ `oklch(0.662 0.188 139)`(Positive) · `red 700 = #DA2128` ≈ `oklch(0.571 0.216 26)`(Hottracks primary) · `red = #EC1F2D` ≈ `oklch(0.603 0.232 26)`(Negative)이고, 100→900 램프는 스와치 **이미지 9장**으로만 노출된다 [src:3]. **네 값 모두 이 문서와 ΔE 0.001로 일치**했고 의미 배정도 맞았다 — 특히 페이지가 단계 없이 `red`로 부르는 Negative 색을 이 문서는 `red-500`으로, Hottracks primary를 `red-700`으로 두어 둘을 구분한다. 나머지 52개는 이 페이지만으로 재검증할 수 없다.
...
- **OKLCH 변환값의 출처가 값마다 다르다 (2026-08-02)** — 원본 시스템은 hex로 게시하지만, Color 페이지가 **텍스트로** 내보내는 값은 코어 4색뿐이다 [src:3] — 그 넷은 ΔE 0.001로 대조됐고(Colors 절 참조), 나머지 램프 값은 스와치 이미지 판독에 의존한다. "모든 색상이 공개 hex를 변환한 것"이라는 종전 서술은 이 구분을 지워 철회한다.
